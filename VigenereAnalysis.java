...
public class VigenereAnalysis {
    // Frequência típica das letras no português
    // Usada para comparar com o texto e inferir deslocamentos
    ...
    static String sanitize(String input) {
        ...
        // Remove tudo que não for letra de a-z
    ...
    static double calcIC(String text) {
        ...
        // IC não faz sentido para textos muito pequenos
        if (n < 2) return 0.0;

        int[] freq = new int[26];

        // Conta frequência de cada letra
        for (char c : text.toCharArray()) freq[c - 'a']++;

        double sum = 0;

        // Soma f*(f-1) para cada letra
        for (int f : freq) sum += (double) f * (f - 1);

        // Fórmula do Índice de Coincidência
    ...
    static double avgIC(String text, int k) {
        ...
        // Divide o texto em k subtextos
        for (int i = 0; i < k; i++) {
            StringBuilder sub = new StringBuilder();

            // Pega letras que foram cifradas com a mesma posição da chave
            for (int j = i; j < text.length(); j += k) sub.append(text.charAt(j));

            total += calcIC(sub.toString());
        }

        // Retorna média dos ICs
        return total / k;
    }

    static int findKeyLength(String cipher) {
        // IC esperado aproximado para português
        double target = 0.072;

        int bestK = 1;
        double bestDiff = Double.MAX_VALUE;

        // Testa tamanhos de chave de 1 até 10
        for (int k = 1; k <= 10; k++) {
            double ic = avgIC(cipher, k);

            System.out.printf("k=%-2d IC_medio=%.4f%n", k, ic);

            // Mede o quão próximo está do português
            double diff = Math.abs(ic - target);

            // Guarda o melhor candidato
            if (diff < bestDiff) {
                bestDiff = diff;
                bestK = k;
            }
    ...
    static int inferShift(String sub) {
        ...
        // Conta frequência das letras no subtexto
        ...
        for (int d = 0; d < 26; d++) {
            ...
            // Compara com distribuição do português
            for (int i = 0; i < 26; i++) {
                score += freq[(i + d) % 26] * PT_FREQ[i];
            }

            // Guarda o melhor deslocamento
            if (score > bestScore) {
                bestScore = score;
                bestShift = d;
            }
    ...
    static String decrypt(String cipher, String key) {
        ...
        for (int i = 0; i < cipher.length(); i++) {
            // Converte letra cifrada para número
            int c = cipher.charAt(i) - 'a';

            // Repete a chave ciclicamente
            int d = key.charAt(i % k) - 'a';

            // Aplica a fórmula de decifração: P = (C - K + 26) mod 26
